...
class ZeusSearch:
    ...
    def parse_bot_heartbeat(self, hb_lines, time_cutoff=None):
        ...
        for line in hb_lines:
            ...
            if line.startswith("Report time:"):
                ...
                if time_cutoff:
                    if data["report_time"] < time_cutoff:
                        return None
    ...
    def init_zeus_data(self, timespan=None):
        if os.path.isdir(self.zeus_path):
            for dir_entry in os.listdir(self.zeus_path):
                ...
                if os.path.isfile(dir_entry_path):
                    self.zeus_data[dir_entry] = self.parse_file_data(dir_entry_path, timespan=timespan)
        elif os.path.isfile(self.zeus_path):
            self.zeus_data[self.zeus_path] = self.parse_file_data(self.zeus_path, timespan=timespan)

    def init_zeus_data_mt(self, timespan=None):
        if os.path.isdir(self.zeus_path):
            for i, dir_entry in enumerate(os.listdir(self.zeus_path)):
                dir_entry_path = os.path.join(self.zeus_path, dir_entry)
                if os.path.isfile(dir_entry_path):
                    thread = threading.Thread(target=self.parse_file_data, args=[dir_entry_path], kwargs={"timespan": timespan})
                    thread.name = i
                    thread.start()
...
if __name__ == '__main__':
    ...
    parser.add_argument("-m", "--multithreaded", action="store_true", default=False, help="Multithread the parsing of files")
    ...
    if args.multithreaded:
        zs.init_zeus_data_mt(timespan)
    else:
        zs.init_zeus_data(timespan)
