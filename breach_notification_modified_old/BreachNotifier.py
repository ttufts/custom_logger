...
class BreachNotifier:
    ...
    def __init__(self, tc_config, breach_path, cache_file, customer_list, verbose=False):
    ...
    def get_customer_domains(self):
        if self.customer_list is not None:
            with open(self.customer_list) as f:
                for line in f:
                    if "@" in line:
    ...
    def write_results_to_txt(self, breach_name, domain, data, outdir="."):
        ...
        with open(breach_txt_filename, "w") as f:
            for email, site in data:
                f.write(u"{}: {}\n".format(email, site).encode("utf-8", "ignore"))
...
if __name__ == '__main__':
    ...
    parser.add_argument("-l", "--customer_list", help="List of customer email addresses")
    ...
    bn = BreachNotifier(config, args.breach_dump, customer_list=args.customer_list, verbose=args.verbose, cache_file=args.cache_file)
